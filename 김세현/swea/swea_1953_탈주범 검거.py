# 지도 - 이차원 배열 형태
# 맨홀 뚜껑으로부터 출발 
# - 터널들을 이동 - 이동 방향: 상하좌우 -> 델타 배열 - 이동 못하는 경우가 존재 - 현재 내 위치에서 뚫려있는 곳만 이동 가능 - 다음 위치의 입구가 뚫려있는 곳으로만 가능
...
def bfs(R, C):
    ...
    while q: # 후보군이 없을 때까지 (더 이상 방문할 수 있는 곳이 없으면 종료)
        ...
        for dir in range(4): # 상하좌우 확인
            ...
            # 현재 상좌 -> next_dirs가 하우가 안뚫렸으면 못간다
            ...
            # 시간을 +1 해주면서 기록
            visited[new_x][new_y] = visited[now_y][now_x] + 1
