...
def shoot(cnt, remains, now_arr):
    ...
    for col in range(W):
        ...
        # copy_arr = [row[:] for row in now_arr] ## 깊은 복사 (행 단위로 복사) # 3차원에서는 안될 수도 있음?...
        copy_arr = copy.deepcopy(now_arr)

        row = -1
        # 가장 위 벽돌을 검색
        for r in range(H):
            if copy_arr[r][col]:  # 벽돌이 있으면
                row = r ## 가장 위의 값으로 바뀌겠지 계속해서
                break
        ...
        # 행, 열, 숫자를 모두 담아야 한다.
        q = deque([(row, col, copy_arr[row][col])]) # r, c, p
        now_remains = remains - 1 # 하나 깨지니까 -1 해주기
        ...
        for c in range(W):
            ...
            for r in range(H - 1, -1, -1):
                if copy_arr[r][c] != 0: # 현재 위치에 벽돌이 있다면
...
for tc in range(1, T + 1):
    ...
    arr = [list(map(int, input().split())) for _ in range(H)] # 현 벽돌 상태
